import { z } from "zod";
import { FL_CAP_RATES_2026, CENTRAL_FL_MARKETS, findCapRateBenchmark, getMarket, } from "../lib/cap-rate-data.js";
export const getCapRateBenchmarksSchema = z.object({
    assetClass: z.string().optional().describe("Filter by asset class, e.g. 'Dollar General', 'multifamily', 'industrial'. Omit to get all."),
    market: z.string().optional().describe("Market name, e.g. 'Orlando', 'Space Coast', 'Lakeland'. Returns market overview when provided."),
});
export function getCapRateBenchmarks(input) {
...
export const calculateCapRateSchema = z.object({
    mode: z.enum(["cap_rate", "price", "noi"]).describe("What to calculate: 'cap_rate' (from NOI and price), 'price' (from NOI and cap rate), 'noi' (from price and cap rate)"),
...
export function calculateCapRate(input) {
    ...
    switch (input.mode) {
        case "cap_rate": {
            if (!input.noi || !input.price)
                return { error: "noi and price are required for mode 'cap_rate'" };
        ...
        case "price": {
            if (!input.noi || !input.capRate)
                return { error: "noi and capRate are required for mode 'price'" };
        ...
        case "noi": {
            if (!input.price || !input.capRate)
                return { error: "price and capRate are required for mode 'noi'" };
