...
const TOOLS = [
  {
    ...
    description:
      "Search MaxLife Realty's commercial real estate marketplace. Filter by property type, location, cap rate, price, square footage, or keyword. Returns active listings with financials and contact info.",
    inputSchema: zodToJsonSchema(searchListingsSchema),
  },
  {
    name: "get_listing",
    description:
      "Get full details on a specific MaxLife Realty listing by its URL slug or UUID. Returns all financials, physical details, tenancy info, contact, and photos.",
    inputSchema: zodToJsonSchema(getListingSchema),
  },
  {
    name: "analyze_deal",
    description:
      "Run a full commercial real estate investment analysis. Calculates IRR, DSCR, cash-on-cash return, equity multiple, deal grade (A+ to D), suggested offer price, and year-by-year cash flows. Supports NNN, Multifamily, and SFR.",
    inputSchema: zodToJsonSchema(analyzeDealSchema),
  },
  {
    name: "analyze_deal_from_pdf",
    description:
      "Upload a deal memo, offering memorandum (OM), rent roll, or property financials PDF and get a full investment analysis. Claude extracts the deal parameters (purchase price, NOI, rent, etc.) and runs the analysis. Provide financing override parameters to customize assumptions.",
    inputSchema: zodToJsonSchema(analyzeDealFromPdfSchema),
  },
  {
    name: "get_cap_rate_benchmarks",
    description:
      "Get Florida 2026 cap rate benchmarks by asset class (NNN tenants, multifamily, industrial, retail, office, medical). Optionally filter by asset class name or get a market overview for a specific Central Florida submarket.",
    inputSchema: zodToJsonSchema(getCapRateBenchmarksSchema),
  },
  {
    name: "calculate_cap_rate",
    description:
      "Simple cap rate calculator. Given any two of: NOI, price, cap rate — calculate the third. Modes: 'cap_rate' (NOI ÷ price), 'price' (NOI ÷ cap rate), 'noi' (price × cap rate).",
    inputSchema: zodToJsonSchema(calculateCapRateSchema),
  },
  {
    name: "capture_lead",
    description:
      "Log a contact's information for MaxLife Realty follow-up. Use this when someone expresses interest in a listing, wants a deal reviewed, or wants to connect with an advisor. Ryan Solberg will reach out directly.",
...
function buildSchema(schema: import("zod").ZodTypeAny): Record<string, unknown> {
  ...
  if (typeName === "ZodDefault") {
    const inner = buildSchema(def.innerType);
    (inner as Record<string, unknown>).default = def.defaultValue();
    if (def.description) (inner as Record<string, unknown>).description = def.description;
    return inner;
  }
  ...
  if (typeName === "ZodEnum") {
    const s: Record<string, unknown> = { type: "string", enum: def.values };
    if (def.description) s.description = def.description;
    return s;
  }
