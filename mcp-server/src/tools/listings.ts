...
export async function searchListings(input: SearchListingsInput) {
  let query = supabase
    .from("listings")
    .select(
      "id, slug, title, property_type, transaction_type, status, city, county, price, cap_rate, noi, building_sqft, units, tenant, lease_type, description, highlights, contact_name, contact_phone, contact_email, photo_urls, created_at"
    )
    .eq("status", "active");
  ...
  if (input.minCapRate != null) query = query.gte("cap_rate", input.minCapRate);
  if (input.maxCapRate != null) query = query.lte("cap_rate", input.maxCapRate);
  if (input.minPrice != null) query = query.gte("price", input.minPrice);
  if (input.maxPrice != null) query = query.lte("price", input.maxPrice);
  if (input.minSqft != null) query = query.gte("building_sqft", input.minSqft);
  if (input.maxSqft != null) query = query.lte("building_sqft", input.maxSqft);
  if (input.search) {
    query = query.or(
      `title.ilike.%${input.search}%,description.ilike.%${input.search}%,tenant.ilike.%${input.search}%`
    );
  }
...
export async function getListing(input: GetListingInput) {
  ...
  const { data, error } = await (filtered.single() as any) as { data: Record<string, unknown> | null; error: { code: string; message: string } | null };
  if (error) {
    if (error.code === "PGRST116") return { error: "Listing not found." };
