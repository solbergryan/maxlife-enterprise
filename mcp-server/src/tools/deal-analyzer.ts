...
export const analyzeDealSchema = z.object({
  assetType: z.enum(["NNN", "Multifamily", "SFR"]).default("NNN").describe(
    "Property type: NNN (triple-net commercial), Multifamily (apartments), or SFR (single-family rental)"
  ),
  ...
  exitCapSpread: z.number().min(-3).max(5).default(0.5).describe(
    "How much the exit cap rate differs from the entry cap rate, in percentage points. Positive = market cools (lower sale price), negative = market heats (higher sale price). E.g. 0.5 is conservative."
  ),
});

export type AnalyzeDealInput = z.infer<typeof analyzeDealSchema>;

export function runDealAnalysis(input: AnalyzeDealInput) {
