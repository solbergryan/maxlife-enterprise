...
export type AssetType = "NNN" | "Multifamily" | "SFR";
...
export interface DealInputs {
  ...
  units?: number;
  // Revenue
  annualRent?: number;           // NNN: total annual rent
  monthlyRentPerUnit?: number;   // MF/SFR: per-unit monthly rent
  rentGrowth?: number;           // percent, e.g. 2.0
  vacancyRate?: number;          // percent, e.g. 5
  // Expenses (MF/SFR)
  opexRatio?: number;            // percent of EGI, e.g. 40
  propertyTaxes?: number;
  insurance?: number;
  repairs?: number;
  mgmtPct?: number;              // percent of EGI
  // Financing
  downPaymentPct?: number;       // percent, e.g. 25
  interestRate?: number;         // percent, e.g. 6.75
  amortization?: number;         // years
  closingCosts?: number;
  loanOrigFee?: number;          // percent, e.g. 1.0
  // Hold & Exit
  holdPeriod?: number;           // years
  exitCapSpread?: number;        // percent, e.g. 0.50
}
...
export interface ScoreDetail {
  stars: number;
  grade: "excellent" | "good" | "fair" | "marginal" | "fail";
  label: string;
}

export interface SuggestedMetrics {
  price: number;
  downPayment: number;
  loanAmount: number;
  equity: number;
...
function scoreIRR(irr: number): ScoreDetail {
...
function scoreDSCR(dscr: number): ScoreDetail {
...
function scoreCoC(coc: number): ScoreDetail {
...
function scoreEM(em: number): ScoreDetail {
...
export function analyzeDeal(raw: DealInputs): DealAnalysis | { error: string } {
  const price = raw.purchasePrice;
  if (!price || price <= 0) return { error: "purchasePrice is required and must be > 0" };
  ...
  if (gpr <= 0) return { error: "Annual rent / monthly rent per unit is required and must be > 0" };
  ...
  const irfCashFlows: number[] = [-totalEquity];
  ...
  for (let yr = 1; yr <= holdYrs; yr++) {
    ...
    if (yr < holdYrs) {
      ...
      let irr = 0;
      try { irr = calcIRR(irfCashFlows); } catch { irr = 0; }
