...
export interface CapRateBenchmark {
  assetClass: string;
  rangeLow: number;  // decimal, e.g. 0.0375
  rangeHigh: number;
  notes?: string;
}

export const FL_CAP_RATES_2026: CapRateBenchmark[] = [
...
export interface MarketSummary {
  ...
  population: string;
  keyDrivers: string[];
  strongPropertyTypes: string[];
  capRateComment: string;
}

export const CENTRAL_FL_MARKETS: MarketSummary[] = [
  {
    name: "Orlando",
...
export function findCapRateBenchmark(query: string): CapRateBenchmark[] {
  const q = query.toLowerCase();
  return FL_CAP_RATES_2026.filter(b =>
    b.assetClass.toLowerCase().includes(q)
  );
}

export function getMarket(query: string): MarketSummary | undefined {
  const q = query.toLowerCase();
  return CENTRAL_FL_MARKETS.find(m =>
